...
-- make the first one explicit and the other three implicit in the definition of collinear 4, etc and use this 
...
@[reducible] def Par (A B C D : Point) := ∃ U V u v X, A ≠ B ∧ C ≠ D ∧ Col A B U ∧ 
...
lemma ray_identity_lemma (A B E : Point) ( H : ( B = B ∨ E = B ∨ B = E) → A ≠ B) :
  Out A B E := sorry 
...
lemma euclidean_preneutral.Cong.flipL {A B C D : Point} : Cong A B C D → Cong B A C D  := sorry

lemma euclidean_preneutral.Cong.flipR {A B C D : Point} : Cong A B C D → Cong A B D C  := sorry

lemma euclidean_preneutral.Cong.flipLR {A B C D : Point} : Cong A B C D → Cong B A D C := sorry
...
lemma collinear1 : ∀ {A B C : Point}, Col A B C → Col B A C :=
  ...
  use or.inl ⟨A⟩,
  cases_type* or,
  repeat {by collinear_identity},
  repeat {by collinear_tac},
  end 
...
lemma collinear4 : ∀ {A B C D : Point}, Col A B C → Col A B D → A ≠ B → Col B C D := sorry
...
begin 
  ...
  cases_type* or, 
    ...
    cases ray1 h₃_h_h_h_h_right_left with h16 h17,
      ...
      repeat {tauto}, simp*,
      cleanup_hyps, 
      --exact ⟨ ABCequalsCBA h5.2.2.2.2.2, ABCequalsCBA h₄a⟩, 
      -- refine ⟨tauto,_,_⟩,
      -- tauto,
      -- have h17:= ray5 h11,
      --   -- have h40: Out B A A := ray4(or.inr(or.inl ⟨A⟩)) ne.symm h₆,
      ...
      use [A, C, a, c , by ray_identity, by ray_identity, by ray_identity, by ray_identity], cleanup_hyps, tauto!,-- h43, h16, h₂, h₅ ], 
...
lemma proposition_06 : ∀ (A B C : Point),
   Triangle A B C → CongA A B C A C B → Cong A B A C :=
begin 
introv h₁ h₂, 
have h₃: ¬ Col A C B,
  intro h, 
...
begin
  ...
  have h28:= collinear4 (collinear4 h25.2.2.1 h23.2.2.1 h17.2) right_2.2.2.2 left.symm,
...
begin 
  ...
  have h4: B ≠ E, 
    ...
    have h6:= collinear4 h3 h4 (raystrict  h_1.1).symm, rcases collinearorder h6 with ⟨_⟩ , 
    contradiction,
  ...
  have h7: ¬Col B A F, 
    ...
    have h8:= collinear4 left_1 right_2_right_left right_2.1.symm,
    have h9:= rayimpliescollinear  h_1.1,
    rcases collinearorder h8 with ⟨_⟩ , rcases collinearorder h9 with ⟨ _ ⟩ ,
    have h11:= collinear4 right_3.1 right_4.2.1 (raystrict  h_1.1).symm, 
...
-- meta def cleanupCong : tactic unit := `[ try { simp only [cong1, cong2, cong3, cong4, cong5, cong6, cong7] }; try { assumption } ]
-- meta def cleanup_hypsCong : tactic unit := `[ try { simp only [cong1, cong2, cong3, cong4, cong5, cong6, cong7] at *} ]
...
have h12: C ≠ Q,
  ...
  have h13: Col A E C:= collinear4 h8 h9 h10.2.2.symm,
  have h14: Col E C A, by collinear_tac,
  have: Per A C M := collinearright h4.2.2.2 (by collinear_tac) h10.2.1,
  tauto!,
  ...
  by_cases (B=H),
    {
      ...
      have h₄: Col B M A := collinear4 h₁ h₂ h61.1,
      by collinear_tac,
    },
    {have h₁: Col B A M := collinear4 h₅ h₆ (ne.symm h),
      by collinear_tac,
     },
tauto,
   ...
   begin
    ...
    have h6 : ¬ Col B E D,
      intro h,
        ...
        have h₆:= collinearorder (collinear4 (collinearorder (collinear4 h h₁ h4.1.symm)).1 h₄ h5.1.symm),
      tauto,
      ...
      have h14: ¬ Col A E D,
        ...
        have h₃:= collinearorder (collinear4 h h₁ h4.2.1),
      tauto,
      have h16: ¬ Col B E C,
        intro h, 
        have h₁:= collinearorder (or.inr(or.inr(or.inr(or.inr(or.inl h1))))),
        have h₃:= collinearorder (collinear4 h h₁.2.2.2.2 h4.1.symm),
      tauto,
...
lemma proposition_18 : ∀ (A B C : Point),
  ...
  begin
    ...
    have h5: ¬ Col B C D,
      ...
      have h₄:= collinear4 h₁.2.2.2.2 (h₃.2.1) h₂.1,
      have h₅:= collinearorder h₄,
    tauto,
    ...
    have h8: ¬ Col A D B,
      ...
      have h₄:= collinear4 h₁.1 (h₃.1) h₂.2.1.symm,
      have h₅:= collinearorder h₄,
    tauto,
...
lemma proposition_20 : ∀ (A B C : Point), Triangle A B C → TG B A A C B C :=
  begin
    ...
    have h6: ¬ Col A D C ,
      ...
      have h3:= collinear4 h1 h2 h4.1.symm,
      tauto,
    have h7:= NCorder h6,
    have h8: C ≠ D,
      intro h,
    ...
    have h13: ¬ Col B C D, 
      ...
      have h₃:= collinearorder(collinear4 h₁ h₂ h4.2.2.symm),
      tauto,
...
begin 
  ...
  have h8: ¬ Col A B Q,
    ...
    have := collinear4 h₂.2.1 h₃.2.2.2 h₄.symm, 
    contradiction,
...
begin 
  ...
  have h13:= collinear4 h11a (h12) h1.1.symm,
  have h13a: Col G A g, by collinear_tac, 
  have h15:= collinear4 h11a h13a h1.1.symm,
  have h16a: Col D C H , by collinear_tac ,
  have h17a: Col H D C , by collinear_tac ,
  have h18: Col D C h := collinear4 h17a h1.2.2.2.2.2.1 h1.2.1,
  cases collinearorder h18 with h19 _,
  have h20: Col D d C := collinear4 h1.2.2.2.2.2.2.1 h17a h9.1.symm,
  have h21: Col C D d, by collinear_tac,
  have h22: ¬ Meet A B C D, 
    rintro ⟨M , h₁⟩,
    rcases collinearorder h₁.2.2.1 with ⟨h₂,_⟩,
    have h₄: Col A G M := collinear4 h30a h₂ h8.2.symm,
    have h₅: Col D H M := collinear4 (collinear1 h16a) h₁.2.2.2 h9.2.symm,
...
begin
  ...
  have h28: Col A B a := collinear4 h24 h26 h16.1.symm,
  cases collinearorder h16.2.2.2.1 with h29 h30,
  have h31: Col A B g := collinear4 h24 h29 h16.1.symm,
  cases collinearorder (or.inr(or.inr(or.inr(or.inl h19.1)))) with h32 h33,
  cases collinearorder h32 with h60 h34,
  have h35: Col D C h := collinear4 h60 h16.2.2.2.2.2.1 h16.2.1,
  cases collinearorder h35 with h36 h37,
  have h38: Col D d C := collinear4 h16.2.2.2.2.2.2.1 h60 h16.2.1,
  cases collinearorder h38 with h39 h40,
  have h41: ¬ Meet A B C D,
    rintro ⟨M, hEq⟩,
    cases collinearorder hEq.2.2.1 with h₁ h₂,
    have h₃: Col A G M := collinear4 h10.2.2.2 h₁ h9.2.symm,
    have h₄: Col D H M := collinear4 h33.2.2.2 hEq.2.2.2 hEq.2.1,
...
lemma proposition_30B :
  ...
  --  have h27: Out G A A := ray4 (or.inr(or.inl ⟨A⟩)) h26.2.1.symm,
   ...
   have h35:= collinear4 h33.2.1 h34 h9.2.2.symm,
   rcases collinearorder h35 with ⟨h36,_⟩,
   have h37a: Col H K m , by collinear_tac,
   have h38:= collinear4 h37a h33.2.2.2 h9.1.symm,
...
/-- The straight lines joining equal and parallel straight lines (at the extremities which are) 
in the same directions (respectively) are themselves also equal and parallel.-/
...
--     have : Meet A B C D, use B, cleanup_hyps, repeat {tauto},
--     have h10:= NCorder h9,
--    have h10:= (ABCequalsCBA h9),

--   --  have h12:= proposition_04 _ _ _ _ _ _ h2.flipL (cn_equalityreverse B C) (h8.trans h10),
--   --  have h13 : CongA A C B C B D := ((ABCequalsCBA h6).symm).trans h8,
--    have h15a : Par A C B D , rw Par, cleanup_hyps, use [B, B, C, C, M], repeat {tauto},--_,_,_,(NCdistinct h6).2.2.1, (NCdistinct h9).2.2.1], 
--    have h15 : Par A C B D := proposition_27B _ _ _ _ h10 ⟨M, h3, collinear1 h5, (NCorder h6).1⟩,
